...
public:
    BIGNUM* get() { return m_bn; }
    const BIGNUM* get() const { return m_bn; }
    ...
    {
        return (unsigned int)BN_get_word(m_bn);
    }
    ...
    void setvch(const std::vector<unsigned char>& vch)
    {
        ...
        BN_mpi2bn(&vch2[0], (int) vch2.size(), m_bn);
    }
    ...
    unsigned int GetCompact() const
    {
        ...
        if (nSize <= 3)
            nCompact = (unsigned int) BN_get_word(m_bn) << 8*(3-nSize);
        else
        {
            CBigNum bn;
            BN_rshift(bn.m_bn, m_bn, 8*(nSize-3));
            nCompact = (unsigned int) BN_get_word(bn.m_bn);
        }
    ...
    operator BIGNUM*() { return m_bn; }
    operator const BIGNUM*() const { return m_bn; }

    // Add friend declarations for all operators
...
{
    CBigNum r;
    if (!BN_add(r.m_bn, a.m_bn, b.m_bn))
        throw bignum_error("CBigNum::operator+ : BN_add failed");
    return r;
}
...
{
    CBigNum r(a);
    BN_set_negative(r.m_bn, !BN_is_negative(r.m_bn));
    return r;
}
...
inline const CBigNum operator*(const CBigNum& a, const CBigNum& b)
{
    CAutoBN_CTX pctx;
    CBigNum r;
    if (!BN_mul(r.m_bn, a.m_bn, b.m_bn, pctx))
        throw bignum_error("CBigNum::operator* : BN_mul failed");
    return r;
}
...
inline const CBigNum operator/(const CBigNum& a, const CBigNum& b)
{
    CAutoBN_CTX pctx;
    CBigNum r;
    if (!BN_div(r.m_bn, NULL, a.m_bn, b.m_bn, pctx))
...
inline const CBigNum operator%(const CBigNum& a, const CBigNum& b)
{
    CAutoBN_CTX pctx;
    CBigNum r;
    if (!BN_mod(r.m_bn, a.m_bn, b.m_bn, pctx))
        throw bignum_error("CBigNum::operator% : BN_div failed");
    return r;
}
...
{
    CBigNum r;
    if (!BN_lshift(r.m_bn, a.m_bn, shift))
        throw bignum_error("CBigNum:operator<< : BN_lshift failed");
    return r;
}
...
inline bool operator==(const CBigNum& a, const CBigNum& b) { return (BN_cmp(a.m_bn, b.m_bn) == 0); }
inline bool operator!=(const CBigNum& a, const CBigNum& b) { return (BN_cmp(a.m_bn, b.m_bn) != 0); }
inline bool operator<=(const CBigNum& a, const CBigNum& b) { return (BN_cmp(a.m_bn, b.m_bn) <= 0); }
inline bool operator>=(const CBigNum& a, const CBigNum& b) { return (BN_cmp(a.m_bn, b.m_bn) >= 0); }
inline bool operator<(const CBigNum& a, const CBigNum& b)  { return (BN_cmp(a.m_bn, b.m_bn) < 0); }
